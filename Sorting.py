...
def timeout_handler(signum, frame):
    raise TimeoutException("Sorting function took too long!")
...
@njit
def partition_numba(arr, low, high, ascending):
    pivot = arr[high]
    i = low - 1
    for j in range(low, high):
...
@njit
def quicksort_numba_impl(arr, low, high, ascending):
    if low < high:
        pi = partition_numba(arr, low, high, ascending)
        quicksort_numba_impl(arr, low, pi - 1, ascending)
        quicksort_numba_impl(arr, pi + 1, high, ascending)
...
def quicksort_numba(df, col_name, ascending=True):
    ...
    # Ensure the array has a precise data type
    if pd.api.types.is_numeric_dtype(df[col_name]):
        arr = arr.astype(np.float64)
    else:
        raise ValueError("Numba sorting only supports numeric data.")

    quicksort_numba_impl(arr, 0, len(arr) - 1, ascending)
    df[col_name] = arr
    end_time = time.time()
...
def visualize_sorting_runtimes(runtime_data):
    """
    Visualize the runtimes of different sorting methods across optimization techniques.
    
    :param runtime_data: List of dictionaries with sorting method, optimization type, and runtime.
    """
    # Convert runtime data to a DataFrame
    df_runtime = pd.DataFrame(runtime_data)

    # Create a bar plot using Seaborn
    plt.figure(figsize=(14, 8))
    sns.barplot(data=df_runtime, x="Sorting Method", y="Runtime (seconds)", hue="Optimization Type")
    plt.title("Sorting Algorithm Runtimes by Optimization Technique", fontsize=16)
    plt.xlabel("Sorting Method", fontsize=14)
    plt.ylabel("Runtime (seconds)", fontsize=14)
    plt.xticks(rotation=45, ha="right", fontsize=12)
    plt.legend(title="Optimization Type", fontsize=12)
    plt.tight_layout()
    plt.show()
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('./Data/amazon_products.csv').sample(100_000, random_state=42)
    col_to_sort = 'title'

    # Turn each title into a numeric value (e.g., sum of character ordinals)
    df[col_to_sort] = df[col_to_sort].apply(lambda x: sum(ord(c) for c in str(x)))

    #print(df[col_to_sort].head())
    ...
    for name, sort_func in SORTING_ALGORITHMS.items():
        runtimes = []
        try:
            for _ in range(100):  # Repeat each method 10 times
                ...
                # Disable the timeout timer after successful execution
                signal.alarm(0)

            # Calculate the average runtime
            avg_runtime = sum(runtimes) / len(runtimes)
            runtime_data.append({
                "Sorting Method": name.split(" (")[0],  # Extract the sorting method name
                "Optimization Type": name.split(" (")[1][:-1],  # Extract the optimization type
                "Runtime (seconds)": avg_runtime
            })
            # Save runtime data to CSV
            runtime_df = pd.DataFrame(runtime_data)
            runtime_df.to_csv('sorting_runtimes.csv', index=False)
            print(f"--- {name} ---")
            print(f"Average Time Taken: {avg_runtime:.6f} seconds\n")

        except TimeoutException:
            print(f"--- {name} TERMINATED ---")
            print(f"Error: Sorting function exceeded {TIMEOUT_LIMIT} seconds.\n")
            runtime_data.append({
                "Sorting Method": name.split(" (")[0],
                "Optimization Type": name.split(" (")[1][:-1],
                "Runtime (seconds)": float('inf')  # Mark as infinite runtime
            })
        except Exception as e:
            print(f"--- {name} FAILED ---")
            print(f"Error: {e}\n")
            runtime_data.append({
                "Sorting Method": name.split(" (")[0],
                "Optimization Type": name.split(" (")[1][:-1],
                "Runtime (seconds)": None  # Mark as failed
            })

    # Visualize the results
    visualize_sorting_runtimes(runtime_data)